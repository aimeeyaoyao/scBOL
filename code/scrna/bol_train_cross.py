...
class scOpen1(nn.Module):
    def __init__(self, input_dim, z_dim, shared_classes, total_classes, num_batches, encodeLayer=[], decodeLayer=[],
                 activation="relu", tau=1.0):
        ...
        self.decoder = buildNetwork([self.z_dim + self.num_batches] + decodeLayer, activation=activation,
                                    batchnorm=False)
...
def extractor(model, test_loader, device):
    ...
    with torch.no_grad():
        for _, data in enumerate(test_loader):
            x_t, label_t, index_t, batch_t = data[0].to(device), data[3].to(device), data[4].to(device), data[5].to(
                device)
...
def main(filename, source_name, target_name, args, device):
    ...
    seen_classes_set, novel_classes_set = class_splitting_cross(filename, source_name, target_name)
    ...
    for epoch in range(args.epochs + 1):
            ...
            if epoch <= args.pretrain:
                ...
                if epoch == args.pretrain:
                    ...
                    for i in range(total_classes):
                        test_distance = torch.matmul(F.normalize(test_embeddings[test_label == i]),
                                                     F.normalize(cluster_centers).t())
                        intra_distance = test_distance[:, i]
                        inter_distance = torch.max(test_distance[:, torch.arange(test_distance.shape[1]) != i], dim=1)[
                            0]
                ...
                overall_acc, seen_acc, unseen_acc, overall_acc2, seen_acc2, unseen_acc2, target_preds, _ = test_new2(
                    model, shared_classes, device,
        ...
        for batch_idx, (x_t, raw_x_t, sf_t, y_t, index_t, batch_t) in enumerate(target_dataloader):
            ...
            if epoch >= args.pretrain:
                ...
                prob_output_ss1 = F.softmax(output_ss1, dim=1)
                prob_output_ss2 = F.softmax(output_ss2, dim=1)

                prob_output_ts1 = F.softmax(output_ts1, dim=1)
                prob_output_ts2 = F.softmax(output_ts2, dim=1)

                prob_output_st1 = F.softmax(output_st1, dim=1)
                prob_output_st2 = F.softmax(output_st2, dim=1)

                prob_output_tt1 = F.softmax(output_tt1, dim=1)
                prob_output_tt2 = F.softmax(output_tt2, dim=1)

                ce_loss = -torch.mean(torch.sum(labeled_onehot * torch.log(prob_output_ss1), dim=1)) \
                          - torch.mean(torch.sum(labeled_onehot * torch.log(prob_output_ss2), dim=1)) \
                          - torch.mean(unlabeled_mask * torch.sum(unlabeled_onehot * torch.log(prob_output_tt1), dim=1)) \
                          - torch.mean(unlabeled_mask * torch.sum(unlabeled_onehot * torch.log(prob_output_tt2), dim=1))

                cluster_loss = -torch.mean(torch.log(F.sigmoid(torch.sum(output_t1[t_idx_near1] * output_t1, dim=1)))) - \
                               torch.mean(torch.log(F.sigmoid(torch.sum(output_t2[t_idx_near2] * output_t2, dim=1)))) - \
                               torch.mean(torch.log(F.sigmoid(torch.sum(output_t1[t_idx_near1] * output_t2, dim=1)))) - \
                               torch.mean(torch.log(F.sigmoid(torch.sum(output_t2[t_idx_near2] * output_t1, dim=1))))
                ...
                if len(train_pseudo_labels_batch[train_pseudo_labels_batch != -1]) != 0:
                    prob_output_st1 = prob_output_st1[train_pseudo_labels_batch != -1]
                    prob_output_st2 = prob_output_st2[train_pseudo_labels_batch != -1]
                    train_pseudo_labels_batch = train_pseudo_labels_batch[train_pseudo_labels_batch != -1]
                    train_pseudo_labels_batch_onehot = F.one_hot(train_pseudo_labels_batch,
                                                                 num_classes=total_classes).float()
                    align_loss = -torch.mean(torch.sum(train_pseudo_labels_batch_onehot * torch.log(prob_output_st1), dim=1)) \
                                 - torch.mean(torch.sum(train_pseudo_labels_batch_onehot * torch.log(prob_output_st2), dim=1))
                if len(test_pseudo_labels_batch[test_pseudo_labels_batch != -1]) != 0:
                    prob_output_ts1 = prob_output_ts1[test_pseudo_labels_batch != -1]
                    prob_output_ts2 = prob_output_ts2[test_pseudo_labels_batch != -1]
                    test_pseudo_labels_batch = test_pseudo_labels_batch[test_pseudo_labels_batch != -1]
                    test_pseudo_labels_batch_onehot = F.one_hot(test_pseudo_labels_batch,
                                                                num_classes=shared_classes).float()
                    align_loss += -torch.mean(torch.sum(test_pseudo_labels_batch_onehot * torch.log(prob_output_ts1), dim=1)) \
                                  - torch.mean(torch.sum(test_pseudo_labels_batch_onehot * torch.log(prob_output_ts2), dim=1))
        ...
        if epoch >= args.pretrain:
            print("Training {}/{}, zinb loss: {:.4f}, ce loss: {:.4f}, "
                  "cluster loss: {:.4f}, align loss: {:.4f}".format(epoch, args.epochs, recon_losses.avg,
                                                                    ce_losses.avg, cluster_losses.avg,
                                                                    align_losses.avg))
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--midtrain', type=int, default=800)  # 800
    ...
    data_infor_set = [["ALIGNED_Homo_sapiens_Placenta", "Vento-Tormo_Smart-seq2", "Vento-Tormo_10x"],
